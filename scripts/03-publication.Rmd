...
pacman::p_load(modelsummary, pwr, survey, broom, dplyr, tidyr, knitr, kableExtra, gt,
               rstatix, ggeffects, ggplot2, gridExtra, magrittr, xtable)
```
...
## Table 1 - Selected policy issues

```{r}
models_pol_issues_sig <- list(
...
msummary(models_pol_issues_sig,
         stars = c('*' = .1, '**' = .05, '***' = .01),
         output = 'markdown')
```
...
## Table 1 - Balance across conditions
...
```{r}
policy_support <- list(
  "Policy Support" = lm(policy_opinion ~ male + pid3 + treatment_group, data = data_long))

msummary(policy_support,
         stars = c('*' = .1, '**' = .05, '***' = .01),
         output = 'markdown')
```


## Table - Treatment effect per policy issues
...
## Figure - Distribution political knowledge
