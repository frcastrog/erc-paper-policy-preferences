...
pacman::p_load(modelsummary, pwr, survey, broom, dplyr, tidyr, knitr, kableExtra, gt,
               rstatix, ggeffects, ggplot2, gridExtra, magrittr, xtable, psych, stringr, ggeffects)
```
...
ate_m2 %<>%
  mutate(
    treatment = case_when(
      ...
      # Exclude baseline by not specifying a condition for it
    ),
    party_id = case_when(
  ...
  # Exclude rows where 'treatment' is NA which corresponds to the baseline
...
ggsave("C:/Users/Francisca/Dropbox/Shared_ERC_Francisca_and_Jenny/r-project/output/figures/ate_party_id.png", 
       ate_plot2, width = 5, height = 4, dpi = 600)
```
...
model3_plot <- ggplot(model3_table, aes(x = x, y = predicted, group = treatment)) +
  ...
  geom_line(color = "black", size = 1) +  # Set line color to black
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
...
ggsave("C:/Users/Francisca/Dropbox/Shared_ERC_Francisca_and_Jenny/r-project/output/figures/ate_sci.png", 
       m4_plot_final, width = 5, height = 4, dpi = 600)
```
...
### Figure C3.1 party ID as in Barber & Pope 2019

```{r}
#regression for average treatment effect among Republicans
bp_republicans <- list(
  "Trump Conservative" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TC'),]),
  "Trump Liberal" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TL'),]),
  "Close Friend Conservative" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFC'),]),
  "Close Friend Liberal" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFL'),]))

msummary(bp_republicans,
         stars = c('*' = .1, '**' = .05, '***' = .01),
         output = 'markdown')
```

```{r}
#regression for average treatment effect among Independents/Other
bp_indep <- list(
  "Trump Conservative" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TC'),]),
  "Trump Liberal" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TL'),]),
  "Close Friend Conservative" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFC'),]),
  "Close Friend Liberal" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFL'),]))
...
bp_democrats <- list(
  "Trump Conservative" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TC'),]),
  "Trump Liberal" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TL'),]),
  "Close Friend Conservative" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFC'),]),
  "Close Friend Liberal" = lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFL'),]))

msummary(bp_democrats,
         stars = c('*' = .1, '**' = .05, '***' = .01),
         output = 'markdown')
```
...
```{r}
# TC treatment
model_tc_1 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TC'),])
model_tc_2 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TC'),])
model_tc_3 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TC'),])

trump.conservative.treat <- c(model_tc_1$coef[3], model_tc_2$coef[3], model_tc_3$coef[3])
trump.conservative.se <- c(coef(summary(model_tc_1))[, "Std. Error"][3], coef(summary(model_tc_2))[, "Std. Error"][3], coef(summary(model_tc_3))[, "Std. Error"][3])
trump.lower.cons <- trump.conservative.treat - 1.96*trump.conservative.se
trump.upper.cons <- trump.conservative.treat + 1.96*trump.conservative.se

# TL Treatment
model_tl_1 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TL'),])
model_tl_2 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TL'),])
model_tl_3 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'TL'),])

trump.liberal.treat <- c(model_tl_1$coef[3], model_tl_2$coef[3], model_tl_3$coef[3])
trump.liberal.se <- c(coef(summary(model_tl_1))[, "Std. Error"][3], coef(summary(model_tl_2))[, "Std. Error"][3], coef(summary(model_tl_3))[, "Std. Error"][3])
trump.lower.liberal <- trump.liberal.treat - 1.96*trump.liberal.se
trump.upper.liberal <- trump.liberal.treat + 1.96*trump.liberal.se

# CFC treatment
model_cfc_1 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFC'),])
model_cfc_2 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFC'),])
model_cfc_3 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFC'),])

cf.conservative.treat <- c(model_cfc_1$coef[3], model_cfc_2$coef[3], model_cfc_3$coef[3])
cf.conservative.se <- c(coef(summary(model_cfc_1))[, "Std. Error"][3], coef(summary(model_cfc_2))[, "Std. Error"][3], coef(summary(model_cfc_3))[, "Std. Error"][3])
cf.lower.cons <- cf.conservative.treat - 1.96*cf.conservative.se
cf.upper.cons <- cf.conservative.treat + 1.96*cf.conservative.se

# CFL Treatment
model_cfl_1 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Republican' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFL'),])
model_cfl_2 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Democrat' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFL'),])
model_cfl_3 <- lm(policy_opinion ~  male + treatment_group, data = data_long[data_long$party_id == 'Independent/Other' & (data_long$treatment_group == 'control' | data_long$treatment_group == 'CFL'),])

cf.liberal.treat <- c(model_cfl_1$coef[3], model_cfl_2$coef[3], model_cfl_3$coef[3])
cf.liberal.se <- c(coef(summary(model_cfl_1))[, "Std. Error"][3], coef(summary(model_cfl_2))[, "Std. Error"][3], coef(summary(model_cfl_3))[, "Std. Error"][3])
cf.lower.liberal <- cf.liberal.treat - 1.96*cf.liberal.se
cf.upper.liberal <- cf.liberal.treat + 1.96*cf.liberal.se


bp_plot_data <- data.frame(
  party = rep(c("Republican", "Democrat", "Independent/Other"), 2),
  treatment = c(rep("Conservative Trump", 3), rep("Liberal Trump", 3), rep("Conservative CF", 3), rep("Liberal CF", 3)),
  estimate = c(trump.conservative.treat, trump.liberal.treat, cf.conservative.treat, cf.liberal.treat),
  lower_ci = c(trump.lower.cons, trump.lower.liberal, cf.lower.cons, cf.lower.liberal),
  upper_ci = c(trump.upper.cons, trump.upper.liberal, cf.upper.cons, cf.upper.liberal))

dodge_width <- 0.2

ggplot(bp_plot_data, aes(x = treatment, y = estimate, color = party, group = party)) +
  geom_point(position = position_dodge(width = dodge_width)) +
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.1, position = position_dodge(width = dodge_width)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  theme_minimal() +
  labs(
    title = "Average Treatment Effect Across Issues",
    x = "Treatment Condition",
    y = "Increased Probability of Voting for Liberal Policy"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5),
    legend.title = element_blank()
  ) +
  scale_color_manual(values = c("Democrat" = "blue", "Independent/Other" = "green", "Republican" = "red"))
